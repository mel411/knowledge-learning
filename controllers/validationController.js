...
exports.validateLesson = async (req, res) => {
  try {
    ...
    // User can access if they bought the lesson or the cursus
    const accessQuery = `
...
`;
    ...
    if (lessonsResult.rows.length === 0) {
      return res.status(500).json({ message: 'Erreur serveur (cursus/lessons)' });
    }
    const lessonIds = lessonsResult.rows.map(l => l.id);
    const cursusId = lessonsResult.rows[0].cursus_id;


    // 5. Count validated lessons for this user in this cursus
    ...
    // 6. If all lessons validated → certificat (strict equality, only once)
    if (validatedCount === lessonIds.length && lessonIds.length > 0) {
      // Check if certificat already exists
      ...
      if (existingCert.rows.length === 0) {
        ...
        return res.json({
          message: "Leçon validée + formation complétée !"
        });
      } else {
        return res.json({
          message: "Leçon validée + formation déjà complétée"
        });
      }
    }
...
exports.getUserValidations = async (req, res) => {
  try {
    const userId = req.user.id;

    const result = await db.query(
      "SELECT lesson_id FROM validations WHERE user_id = $1",
      [userId]
    );
    ...
    console.error("getUserValidations error:", error);
    return res.status(500).json({ 
      message: "Erreur serveur" 
    });
  }
